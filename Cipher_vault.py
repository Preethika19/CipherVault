...
tables_to_drop = ["AccessTime", "Passwords"]
drop_tables(tables_to_drop)
'''
#Creation of MySQL tables to store passwords and accesstime
connection = mydb
cursor = connection.cursor()
create_query = "CREATE TABLE IF NOT EXISTS Passwords (Title VARCHAR(50) ,passwords VARCHAR(50) )"     
cursor.execute(create_query)
connection.commit()
check_query = "SELECT COUNT(*) FROM Passwords"
...
cursor.execute(create_query)
connection.commit()
check_query = "SELECT COUNT(*) FROM AccessTime"
cursor.execute(check_query)
result = cursor.fetchone()
if result:
    count_records = result[0]
